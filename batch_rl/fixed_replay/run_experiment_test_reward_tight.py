...
class Elem(object):
    def __init__(self, snapshot, state, a, rho, re, no, node_id):
        global global_id
        self.gid = global_id
        global_id += 1
...
class TestRunner(object):
  ...
  def take_action(self, state, rho_lim, re_cur, no, node_id):
      ...
      for a in range(self.num_actions):
          ...
          if rho <= rho_lim:
            ...
            elem = Elem(snapshot, state, a, rho, re_cur, no, node_id)
  ...
  def expand(self, state, rho_lim=0, re_cur=0, no=0, node_id=0):
    state_bytes = state.tobytes()
    ...
    a_list = self.take_action(state, rho_lim, re_cur, no, node_id)
    ...
    for a in a_list:
      ...
      next_dict = self.expand(next_state, rho_lim, re_cur+reward, no+1, self.node_id)
  ...
  def get_first(self, queue):
      elem = queue.pop(0)
      return elem.snapshot, elem.state, elem.a, elem.rho, elem.re, elem.no, elem.gid, elem.node_id
  ...
  def run_experiment(self):
    ...
    self.expand(state, rho_lim=0, re_cur=0, no=0, node_id=0)
    ...
    while 1:
      ...
      snapshot, state, a, rho, re, no, gid, node_id = self.get_first(cur_queue)
      ...
      self.expand(next_state, rho, re+reward, no+1, self.node_id)
