...
from batch_rl.fixed_replay import atari_lib_features
...
class DQNAgent(object):
  ...
  def __init__(self,
               ...
               network=atari_lib_features.NatureDQNFeatNetwork,
               gamma=0.99,
               update_horizon=1,
    ...
    with tf.device(tf_device):
      ...
      state_shape_any = (None,) + self.observation_shape + (stack_size,)
      
      self.state = np.zeros(state_shape)
      self.state_ph = tf.compat.v1.placeholder(
          self.observation_dtype, state_shape, name='state_ph')
      self.state_ph_any = tf.compat.v1.placeholder(
          self.observation_dtype, state_shape_any, name='state_ph')
  ...
  def _build_networks(self):
    ...
    self._net_outputs, self._hidden = self.online_convnet(self.state_ph)

    self._net_outputs_any, self._hidden_any = self.online_convnet(self.state_ph_any)

    # TODO(bellemare): Ties should be broken. They are unlikely to happen when
    # using a deep network, but may affect performance with a linear
    # approximation scheme.
    self._q_max_any = tf.reduce_max(self._net_outputs_any.q_values, axis=1)
    self._q_argmax = tf.argmax(self._net_outputs.q_values, axis=1)[0]

    if not self.eval_mode:
      self._replay_net_outputs, self._replay_hidden = self.online_convnet(self._replay.states)
      self._replay_next_target_net_outputs, self._replay_next_target_hidden = self.target_convnet(
  ...
  def _select_action(self):
    ...
    #   return self._sess.run(self._q_argmax, {self.state_ph: self.state})
      q_argmax, hidden = self._sess.run([self._q_argmax, self._hidden], {self.state_ph: self.state})

    return q_argmax


  def get_hidden(self, state):
      hidden = self._sess.run(self._hidden, {self.state_ph: state})
      return hidden

  def get_hidden_any(self, state):
      hidden = self._sess.run(self._hidden_any, {self.state_ph_any: state})
      return hidden

  def get_q_max_any(self, state):
      q_max = self._sess.run(self._q_max_any, {self.state_ph_any: state})
      return q_max
