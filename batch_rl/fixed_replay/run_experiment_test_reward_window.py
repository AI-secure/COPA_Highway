...
class Elem(object):
    def __init__(self, snapshot, state, action_seqs, a, rho, re, no, node_id):
        global global_id
        self.gid = global_id
        global_id += 1
...
class TestRunner(object):
  ...
  def __init__(self,
    ...
    self._logger.info('queue files: starting from scratch')
  ...
  def _create_directories(self):
    ...
    setup_logger('window reward cert', log_file)
  ...
  def _update_action_seqs(self, action_seqs, actions):
    next_action_seqs = np.delete(action_seqs, 0, 0) if len(action_seqs) == self._window_size else action_seqs
  ...
  def _compute_action_and_certifications(self, actions):
    ...
    action = np.argmax(n)
    # self._logger.info(f'[compute_action_and_certifications] n arr = {n}')
    ...
    # self._logger.info(f'[compute_action_and_certifications] c mat = {c}')
    ...
    for a in range(self.num_actions):
      if a == action:
        ...
        poison_sizes[a] = self._compute_poison_size(action, a, n, c, W=len(actions_arr))

    return action, poison_sizes

  def take_action(self, state, action_seqs, rho_lim, re_cur, no, node_id):
      ...
      a_star, poison_sizes = self._compute_action_and_certifications(next_action_seqs)
      ...
      for a in range(self.num_actions):
          ...
          if rho <= rho_lim:
            ...
            elem = Elem(snapshot, state, next_action_seqs, a, rho, re_cur, no, node_id)
  ...
  def expand(self, state, action_seqs, rho_lim=0, re_cur=0, no=0, node_id=0):
    state_bytes = state.tobytes()
    ...
    a_list, next_action_seqs = self.take_action(state, action_seqs, rho_lim, re_cur, no, node_id)
    ...
    for a in a_list:
      ...
      self.node_id += 1
      self.edges.append((node_id, self.node_id, a, reward, rho_lim))
      ...
      next_dict = self.expand(next_state, next_action_seqs, rho_lim, re_cur+reward, no+1, self.node_id)
  ...
  def get_first(self, queue):
      elem = queue.pop(0)
      return elem.snapshot, elem.state, elem.action_seqs, elem.a, elem.rho, elem.re, elem.no, elem.gid, elem.node_id
  ...
  def run_experiment(self):
    ...
    self.expand(state, action_seqs, rho_lim=0, re_cur=0, no=0, node_id=0)
    ...
    while 1:
      while not cur_queue:
        ...
        if global_rho == self._total_num // 2 + 2:
          ...
          save_filename = os.path.join(self._base_dir, 'edges.pt')
          torch.save(self.edges, save_filename)
          self._logger.info(f'tree edges saved to {save_filename}')
      ...
      snapshot, state, next_action_seqs, a, rho, re, no, gid, node_id = self.get_first(cur_queue)
      ...
      self.node_id += 1
      self.edges.append((node_id, self.node_id, a, reward, rho))
      ...
      self.expand(next_state, next_action_seqs, rho, re+reward, no+1, self.node_id)
