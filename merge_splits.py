...
output_folder = osp.join(osp.join(args.input_folder, f'hash_{"%02d"%args.hash_num}'), 'replay_logs')
if not osp.exists(output_folder):
    os.makedirs(output_folder)

task_name = 'merge partitioned data'
setup_logger(task_name, osp.join(osp.join(args.input_folder, f'hash_{"%02d"%args.hash_num}'), 'gen.log'))
...
for i in range(0, 20):
    logger.info(f'--------------------- dataset {"%02d"%i} ----------------------------------')

    folder = f'{args.input_folder}_{"%02d"%i}/dataset/hash_{"%02d"%args.hash_num}/replay_logs'
    ...
    while 1:
        ...
        logger.info(f'concatenating done using {time.time() - start} seconds! previous size = {len(prev_ob)}, current size = {len(ob)}')
        ...
        if len(ob) >= SPLIT_SIZE:
            ...
            write_gz_file(np.asarray([14998, 14999], dtype=np.int64), osp.join(output_folder, f'invalid_range_ckpt.{gid}.gz'))
            ...
            logger.info(f'saving gid {gid} done using {time.time() - start} seconds! remaining size = {len(prev_ob)}')
...
if len(ob):
    ...
    count = len(ob)
    write_gz_file(np.asarray([count-1, count], dtype=np.int64), osp.join(output_folder, f'invalid_range_ckpt.{gid}.gz'))
    write_gz_file(np.asarray(count, dtype=np.int64), osp.join(output_folder, f'add_count_ckpt.{gid}.gz'))
