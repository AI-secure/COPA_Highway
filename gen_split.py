...
def parse_args():
    ...
    parser.add_argument('--replay-buffer-capacity', type=int, default=15000)
...
new_ob = np.empty((0, 5, 5), dtype=np.uint8)
...
for cur_epi in range(args.start_id, args.end_id):
    ...
    if len(new_ind):
        ...
        while len(new_action) >= args.replay_buffer_capacity:
            logger.info('************************************************************')
            logger.info(f'start saving {wid} with len = {len(new_action)}!')

            start = time.time()
            write_gz_file(np.roll(new_ob[:args.replay_buffer_capacity], -1, axis=0), osp.join(output_folder, f'$store$_observation_ckpt.{wid}.gz'))
            logger.info(f'write observation {wid} using {time.time() - start} seconds!')


            write_gz_file(np.roll(new_reward[:args.replay_buffer_capacity], -1), osp.join(output_folder, f'$store$_reward_ckpt.{wid}.gz'))
            write_gz_file(np.roll(new_action[:args.replay_buffer_capacity], -1), osp.join(output_folder, f'$store$_action_ckpt.{wid}.gz'))
            write_gz_file(np.roll(new_terminal[:args.replay_buffer_capacity], -1), osp.join(output_folder, f'$store$_terminal_ckpt.{wid}.gz'))

            new_ob = new_ob[args.replay_buffer_capacity:]
            new_reward = new_reward[args.replay_buffer_capacity:]
            new_action = new_action[args.replay_buffer_capacity:]
            new_terminal = new_terminal[args.replay_buffer_capacity:]
