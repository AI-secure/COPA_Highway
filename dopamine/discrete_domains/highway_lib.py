...
HIGHWAY_MIN_VALS = None
HIGHWAY_MAX_VALS = None
gin.constant('highway_lib.HIGHWAY_OBSERVATION_SHAPE', (5, 5))
gin.constant('highway_lib.HIGHWAY_OBSERVATION_DTYPE', tf.float64)
gin.constant('highway_lib.HIGHWAY_STACK_SIZE', 1)
...
@gin.configurable
class HighwayDQNNetwork(tf.keras.Model):
  ...
  def __init__(self, num_actions, name=None):
    ...
    Args:
      ...
      name: str, used to create scope for network parameters.
    """
    super(HighwayDQNNetwork, self).__init__(name=name)
    self.net = BasicDiscreteDomainNetwork(
        HIGHWAY_MIN_VALS, HIGHWAY_MAX_VALS, num_actions)
...
@gin.configurable
class HighwayRainbowNetwork(tf.keras.Model):
  ...
  def __init__(self, num_actions, num_atoms, support, name=None):
    ...
    Args:
      ...
      name: str, used to create scope for network parameters.
    """
    super(HighwayRainbowNetwork, self).__init__(name=name)
    self.net = BasicDiscreteDomainNetwork(
        HIGHWAY_MIN_VALS, HIGHWAY_MAX_VALS, num_actions, num_atoms=num_atoms)
    self.num_actions = num_actions
    self.num_atoms = num_atoms
...
@gin.configurable
class HighwayQuantileNetwork(tf.keras.Model):
  """Keras Quantile network for Cartpole."""

  def __init__(self, num_actions: int, num_atoms: int, name=None):
    """Builds the deep network used to compute the agent's Q-values.
    ...
    Args:
      ...
      name: str, used to create scope for network parameters.
    """
    super(HighwayQuantileNetwork, self).__init__(name=name)
    self.net = BasicDiscreteDomainNetwork(
        HIGHWAY_MIN_VALS, HIGHWAY_MAX_VALS, num_actions, num_atoms=num_atoms)
    self.num_actions = num_actions
    self.num_atoms = num_atoms
